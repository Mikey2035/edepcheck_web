...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      // If requesting only the latest exam
      ...
      // 🔄 Update total_examinees in tb_exam from responses
      await pool.query(`
        UPDATE tb_exam e
        LEFT JOIN (
          SELECT exam_code_id, COUNT(*) AS count
          FROM responses
          GROUP BY exam_code_id
        ) r ON e.id = r.exam_code_id
        SET e.total_examinees = IFNULL(r.count, 0)
      `);

      // 📊 Fetch exams with computed severity
      const [rows]: any = await pool.query(`
        SELECT 
          e.exam_code, 
          e.title, 
